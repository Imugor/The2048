...
#os.system('pyuic5 The2048_window.ui -o The2048_window.py')

from The2048Class import The2048
from The2048_window import Ui_MainWindow
...
class MyWindow(Qt.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.action6_6.triggered.connect(lambda: self.set_n_cells(6))
        self.ui.action7_7.triggered.connect(lambda: self.set_n_cells(7))
        self.ui.action8_8.triggered.connect(lambda: self.set_n_cells(8))
    ...
    def create_window_labels(self, first=0):
        ...
        for i in range(self.n_cells):
            ...
            for j in range(self.n_cells):
                self.labels[i].append(QtWidgets.QLabel(self.centralWidget()))
                self.labels[i][j].setStyleSheet("""background-color: rgb(255,255,255);
                                                   border: 1px solid;""")
                self.labels[i][j].setGeometry(Qt.QRect(first_pixel_x + (size_button_value + self.padding) * j,
                                                       first_pixel_y + (size_button_value + self.padding) * i,
                                                       size_button_value, size_button_value))
                self.labels[i][j].setFont(Qt.QFont('Sitka Text', size_button_value // 5))
        ...
        if first == 0:
            self.game = The2048(self.n_cells, self.n_cells)
            self.points = 0
        self.show_grid(0)
